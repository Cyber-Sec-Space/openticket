  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight flex items-center">
            <Bug className="mr-3 text-red-500/30 h-8 w-8 animate-pulse" /> 
      ...
      <div className="glass-card rounded-xl p-4 flex gap-4 items-center mb-6 border border-border">
        ...
        <div className="h-9 w-[120px] bg-red-500/10 rounded-md animate-pulse"></div>
      </div>
        ...
        <div className="divide-y divide-border">
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="flex flex-row items-center w-full min-h-[64px] px-6 gap-6 py-2">
              <div className="flex-none h-5 w-32 bg-white/10 rounded-md animate-pulse"></div>
              <div className="flex-1 flex flex-col gap-2 ml-4">
                 <div className="h-4 w-3/4 max-w-[400px] bg-white/10 rounded animate-pulse"></div>
              </div>
              <div className="flex-none hidden md:block h-6 w-16 bg-white/10 rounded-full animate-pulse"></div>
              ...
              <div className="flex-none h-4 w-24 bg-white/5 rounded animate-pulse ml-auto text-right"></div>
            </div>
          ))}
        </div>
        {/* Pagination Skeleton */}
