import { Activity, BarChart3, ScanFace, ShieldAlert, Target, LayoutList, Server, Users, AlertTriangle, Bug, Command } from "lucide-react"
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight flex items-center">
            <div className="h-10 w-48 bg-white/10 rounded-md animate-pulse"></div>
          </h1>
          <div className="h-5 w-64 bg-white/5 rounded-md mt-3 animate-pulse"></div>
        </div>
        <div className="flex gap-4">
          <div className="px-4 py-2 rounded-full border border-primary/20 flex items-center bg-primary/5 text-primary/50 animate-pulse">
            <Activity className="w-4 h-4 mr-2 opacity-50" /> <div className="h-4 w-8 bg-primary/20 rounded"></div>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="glass-card p-4 flex flex-col justify-between rounded-xl relative overflow-hidden border border-white/5 h-[120px]">
      ...
      <div className="mt-8 grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="col-span-1 xl:col-span-2 flex flex-col gap-8">
          ...
          <div className="glass-card rounded-xl border border-white/5 overflow-hidden shadow-2xl flex flex-col">
            <div className="p-5 border-b border-border/50 bg-black/20 flex items-center justify-between">
              <h3 className="text-white/30 font-semibold tracking-wide flex items-center gap-2">
            ...
            <div className="p-6 flex-1 flex flex-col justify-center min-h-[320px]">
               ...
               <div className="w-full h-full flex items-end justify-between gap-2 opacity-20">
                  {Array.from({ length: 14 }).map((_, j) => (
                     <div key={j} className="w-full bg-white/10 rounded-t" style={{ height: `${Math.random() * 60 + 20}%`}}></div>
                  ))}
               </div>
            </div>
          </div>
        ...
        <div className="col-span-1 flex flex-col gap-6">
          ...
          <div className="border border-white/5 bg-black/20 rounded-xl flex flex-col justify-between p-5 shadow-[0_0_20px_rgba(255,255,255,0.02)] shrink-0">
            <div className="h-4 w-32 bg-white/10 rounded tracking-widest uppercase mb-3 animate-pulse"></div>
          ...
          <div className="glass-card rounded-xl border border-white/5 overflow-hidden shadow-2xl flex flex-col flex-1">
            <div className="p-4 border-b border-border/50 bg-black/20 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <div className="h-4 w-16 bg-white/10 rounded-full animate-pulse"></div>
              </div>
               ...
               {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="block p-4 relative overflow-hidden">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-1">
                      <div className="h-4 w-3/4 max-w-[200px] bg-white/10 rounded animate-pulse"></div>
                      <div className="h-4 w-16 bg-white/5 rounded-sm animate-pulse"></div>
                    </div>
