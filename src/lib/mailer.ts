 ...
 * All user-supplied content MUST be passed through this before embedding in HTML.
 */
...
async function getTransporter() {
  ...
  return nodemailer.createTransport({
    ...
    tls: {
      // Security: Validate TLS certificates by default to prevent MITM attacks.
      // Self-signed certs in dev/staging should use proper CA or set smtpTlsRejectUnauthorized via DB.
      rejectUnauthorized: (settings as any).smtpTlsRejectUnauthorized ?? true
    }
  })
}
...
export async function sendCriticalAlertEmail(incident: { id: string, title: string, description: string, severity: string, status: string }, targetEmails: string[]) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p><strong>Title:</strong> ${escapeHtml(incident.title)}</p>
      <p><strong>Severity:</strong> <span style="background-color: #ff3333; color: white; padding: 2px 6px; border-radius: 4px; font-weight: bold;">${escapeHtml(incident.severity)}</span></p>
      ...
      <div style="background-color: #222; padding: 15px; border-left: 4px solid #ff3333; margin-top: 15px;">
        <p style="margin: 0; font-family: monospace; white-space: pre-wrap;">${escapeHtml(incident.description)}</p>
      </div>
...
export async function sendIncidentAssignmentEmail(incidentId: string, incidentTitle: string, userEmail: string, userName: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p><strong>Title:</strong> ${escapeHtml(incidentTitle)}</p>
...
export async function sendResolutionEmail(incidentId: string, incidentTitle: string, reporterEmail: string, reporterName: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p>Hello <strong>${escapeHtml(reporterName)}</strong>,</p>
...
export async function sendAssetCompromisedEmail(assetName: string, assetIp: string, targetEmails: string[]) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p><strong>Asset Name:</strong> ${escapeHtml(assetName)}</p>
      <p><strong>IP Address:</strong> <span style="font-family: monospace;">${escapeHtml(assetIp || 'Unknown')}</span></p>
      <p style="margin-top: 20px; font-size: 12px; color: #888;">This is an automated operational alert generated by OpenTicket.</p>
    </div>
  `
  await transporter.sendMail({
...
export async function sendNewRegistrationAlertEmail(userEmail: string, userName: string, targetEmails: string[]) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p><strong>Operator Name:</strong> ${escapeHtml(userName)}</p>
      <p><strong>Email Address:</strong> ${escapeHtml(userEmail)}</p>
...
export async function sendNewVulnerabilityAlertEmail(vulnTitle: string, severity: string, targetEmails: string[]) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 20px; border-radius: 8px;">
      ...
      <p><strong>Vulnerability:</strong> ${escapeHtml(vulnTitle)}</p>
      <p><strong>Severity:</strong> <span style="font-weight: bold;">${escapeHtml(severity)}</span></p>
...
export async function sendVerificationEmail(email: string, userName: string, tokenUrl: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 30px; border-radius: 8px;">
      ...
      <p>Welcome to OpenTicket, <strong>${escapeHtml(userName)}</strong>.</p>
      <p>Before you can construct authentication tokens or access the dashboard, you must verify your identity routing.</p>
      
      <div style="margin: 30px 0; text-align: center;">
        <a href="${tokenUrl}" style="background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Verify Email Address</a>
      </div>
      
      <p style="font-size: 13px; color: #a3a3a3;">Or manually execute the following payload link in your browser:</p>
      <p style="font-size: 11px; word-break: break-all; background-color: #000; padding: 10px; border: 1px solid #333; color: #34d399; font-family: monospace;">${tokenUrl}</p>
...
export async function sendPasswordResetEmail(email: string, resetUrl: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 30px; border-radius: 8px;">
      ...
      <div style="margin: 30px 0; text-align: center;">
        <a href="${resetUrl}" style="background-color: #ef4444; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Reset Password Overlay</a>
      </div>
      
      <p style="font-size: 13px; color: #a3a3a3;">Or manually execute the following payload link in your browser:</p>
      <p style="font-size: 11px; word-break: break-all; background-color: #000; padding: 10px; border: 1px solid #333; color: #f87171; font-family: monospace;">${resetUrl}</p>
...
export async function sendOperatorInvitationEmail(email: string, joinUrl: string, inviterName: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #444; background-color: #111; color: #fff; padding: 30px; border-radius: 8px;">
      ...
      <p>Administrator <strong>${escapeHtml(inviterName)}</strong> has provisioned a secure invitation for you to join the OpenTicket platform.</p>
      
      <div style="margin: 30px 0; text-align: center;">
        <a href="${joinUrl}" style="background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Accept Invitation</a>
      </div>
      
      <p style="font-size: 13px; color: #a3a3a3;">Or manually execute the following payload link in your browser:</p>
      <p style="font-size: 11px; word-break: break-all; background-color: #000; padding: 10px; border: 1px solid #333; color: #34d399; font-family: monospace;">${joinUrl}</p>
