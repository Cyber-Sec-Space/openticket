...
### 2.4 Hybrid Plugin Architecture & EventBus
...
Furthermore, OpenTicket supports a **Hybrid Registry Distribution Model**, empowering Administrators to browse and install remote capabilities directly from the UI without manually writing code.
```mermaid
graph LR
    SystemEvents[Data Mutation / Creation] --> HookEngine((Hook Engine EventBus))
    HookEngine --> DBCheck{Check `PluginState` DB }
    DBCheck -- "Activated" --> Plugins[Invoke Bound Plugins isolated from main thread]
    
    subgraph Registry [Remote Distribution]
        RemoteRepo[GitHub Raw Registry JSON] -->|Server Action| Download[Fetch & Inject .tsx]
        Download --> Build[Compile webpack production bundle]
        Build --> Restart[Zero-Downtime Hot Reload Process]
    end
```
